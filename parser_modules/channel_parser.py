...
class ChannelParser:
    ...
    async def _get_posts(self) -> List[Dict[str, Any]]:
        """Get posts from the channel with improved scrolling and date filtering."""
        posts = []
        ...
        try:
            # Scroll up to load older posts (in Telegram Web, newest posts are at the bottom)
            processed_ids = set()
            ...
            await self.page.evaluate('''
                () => {
                    // Initialize scroll position to load initial messages
            ...
            await asyncio.sleep(2)  # Let initial posts load
            
            # Process posts in batches with optimized scrolling
            while len(posts) < post_limit and consecutive_no_new_posts < max_no_new_attempts:
                # Scroll up in a batch before processing
                for _ in range(scroll_batch_size):
                    await self.page.evaluate('''
                        () => {
                            const chatContainer = document.querySelector('.chat-container');
                            if (chatContainer) {
                                chatContainer.scrollTop -= 800;  // Scroll up by 800px
                            }
                        }
                    ''')
                    scroll_count += 1
                    await asyncio.sleep(0.2)  # Small pause between scrolls
                
                # Allow content to load
                await asyncio.sleep(1)
                
                # Get all visible message elements
                message_elements = await self.page.querySelectorAll('.message')
                
                if not message_elements:
                    logger.debug("No message elements found with current selector, trying alternative...")
                    # Try alternative selectors
                    message_elements = await self.page.querySelectorAll('.bubble')
                ...
                for msg_element in message_elements:
                    try:
                        ...
                        if not await self._is_date_in_range(msg_element):
                            logger.debug(f"Message {msg_id} outside date range, skipping")
                            
                            # If we found an older message than our start date, we can stop scrolling
                            if self.config.start_date:
                                date_element = await msg_element.querySelector('.time, .date')
                                if date_element:
                                    date_text = await self.page.evaluate('(element) => element.textContent', date_element)
                                    logger.debug(f"Found message with date {date_text} before start date")
                                    
                                    # Check if we've reached older messages than our start date
                                    timestamp = await self.page.evaluate('''
                                        (element) => {
                                            const timeElem = element.querySelector('[data-timestamp]');
                                            if (timeElem) {
                                    ...
                                    if timestamp:
                                        ...
                                        if msg_date.date() < self.config.start_date:
                                            logger.info(f"Reached messages older than start date, stopping scroll")
